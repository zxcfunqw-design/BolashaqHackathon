...
import {
  createOpenAiActionPlanRequest,
  createOpenAiImplementationPlan,
  type OpenAiActionPlanRequest
} from "./src/frontend/lib/actionPlanAi";
import {
  createOpenAiPersonalizedGraph,
  createPersonalizedGraphRequest,
  type OpenAiPersonalizedGraphRequest
} from "./src/frontend/lib/personalizedGraph";
...
export default defineConfig(({ mode }) => {
  ...
  return {
    plugins: [
      ...
      personalizedGraphApiPlugin(openAiApiKey, openAiModel),
      actionPlanApiPlugin(openAiApiKey, openAiModel)
    ],
    server: {
      host: "127.0.0.1",
      proxy: {
...
function personalizedGraphApiPlugin(openAiApiKey: string | undefined, openAiModel: string): Plugin {
  return {
    name: "qadamgraph-personalized-graph-api",
    configureServer(server) {
      server.middlewares.use("/api/personalized-graph", async (request, response) => {
        if (request.method !== "POST") {
          sendJson(response, 405, { error: { message: "Method not allowed" } });
          return;
        }
        ...
        try {
          const body = (await readJsonBody(request)) as OpenAiPersonalizedGraphRequest;

          if (!body.user || !body.baseGraph?.nodes || !body.baseGraph?.edges || !body.universities) {
            sendJson(response, 400, {
              error: { message: "Personalized graph context is missing." }
            });
            return;
          }
          ...
          const openAiResponse = await fetch(OPENAI_RESPONSES_URL, {
            ...
            body: JSON.stringify(createPersonalizedGraphRequest(body.user, body.baseGraph, body.universities, openAiModel))
          });
          ...
          sendJson(response, 200, createOpenAiPersonalizedGraph(text, body));
        } catch (error) {
          const message = error instanceof Error ? error.message : "Personalized graph generation failed.";
          sendJson(response, 500, { error: { message } });
        }
      });
    }
...
function actionPlanApiPlugin(openAiApiKey: string | undefined, openAiModel: string): Plugin {
  return {
    name: "qadamgraph-action-plan-api",
    configureServer(server) {
      server.middlewares.use("/api/action-plan", async (request, response) => {
        if (request.method !== "POST") {
          sendJson(response, 405, { error: { message: "Method not allowed" } });
          return;
        }
        ...
        try {
          const body = (await readJsonBody(request)) as OpenAiActionPlanRequest;

          if (!body.selectedNode || body.selectedNode.type !== "action" || !body.path) {
            sendJson(response, 400, {
              error: { message: "Action node context is missing." }
            });
            return;
          }
          ...
          const openAiResponse = await fetch(OPENAI_RESPONSES_URL, {
            ...
            body: JSON.stringify(createOpenAiActionPlanRequest(body, openAiModel))
          });
          ...
          sendJson(response, 200, createOpenAiImplementationPlan(text, body));
        } catch (error) {
