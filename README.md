...
The portfolio screen calls a local Vite API endpoint, which then calls the OpenAI Responses API.
Create `.env.local`:
...
Restart `npm.cmd run dev` after changing `.env.local`. If the key is missing or the request
