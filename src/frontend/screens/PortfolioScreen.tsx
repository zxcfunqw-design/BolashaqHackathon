import { AlertCircle, Check, Clipboard, FileDown, Loader2, WandSparkles } from "lucide-react";
...
const PortfolioPdfDownload = lazy(async () => {
  const module = await import("../components/PortfolioPdfDownload");
  return { default: module.PortfolioPdfDownload };
});
...
export function PortfolioScreen({
  ...
  const pdfPortfolio = useMemo<PortfolioDraft>(
    () => ({
      ...portfolio,
      fields,
      output: draft?.text ?? portfolio.output ?? createPortfolioOutput(fields),
      generated: Boolean(draft ?? portfolio.generated),
      updatedAt: draft?.createdAt ?? portfolio.updatedAt,
      source: draft?.source ?? portfolio.source,
      model: draft?.model ?? portfolio.model,
      warning: draft?.warning ?? portfolio.warning
    }),
    [draft, fields, portfolio]
  );
  ...
  return (
    <div className="space-y-4">
      ...
      {draft ? (
        <Card className="border-qadam-primary/20 bg-emerald-50 dark:bg-emerald-500/10">
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center justify-end gap-2">
              ...
              <Suspense
                fallback={
                  <span className={downloadButtonClassName(true)}>
                    <FileDown size={16} />
                    Loading PDF...
                  </span>
                }
              >
                <PortfolioPdfDownload
                  fileName={buildPdfFileName(pdfPortfolio.fields.studentName)}
...
function buildPdfFileName(studentName: string) {
  const safeName = studentName.trim().replace(/\s+/g, "_").replace(/[^\w.-]/g, "");
  return `Portfolio_${safeName || "Student"}.pdf`;
}
